...
object NaiveBayesExample {
  def main(args: Array[String]): Unit = {
    ...
    val trainLabeFilePath = userPath + "/src/main/resources/data/train-labels.idx1-ubyte"
    val trainImageFilePath = userPath + "/src/main/resources/data/train-images.idx3-ubyte"
    val testLabeFilePath = userPath + "/src/main/resources/data/t10k-labels.idx1-ubyte"
    ...
    val trainLabe = loadLabel(trainLabeFilePath)
    val trainImages = loadImages(trainImageFilePath)
    val testLabe = loadLabel(testLabeFilePath)
    val testImages = loadImages(testImageFilePath)


    //处理成mlLib能用的基本类型 LabeledPoint
    if(trainLabe.length == trainImages.length) {
      //标签数量和图像数量能对上则合并数组  Array[(labe,images)]

      trainLabe.zip(trainImages).take(1).map( d =>{
        println("图片标签：" + d._1.toInt)
        var i = 0;
        d._2.map(p =>{
          print((p & 0xFF).toDouble + "   ")
          i = i + 1
          if(i == 28) {
            println()
            i = 0
